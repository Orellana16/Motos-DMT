<section>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        <div>
            ...
            <x-text-input id="pac-input" class="mt-1 block w-full" type="text" placeholder="Empieza a escribir tu calle..." />
        </div>

        {{-- Contenedor del Mapa --}}
        <div id="map" style="height: 400px; width: 100%;" class="mt-4 rounded-md shadow-sm border border-gray-300"></div>

        {{-- Campos de Geoposicionamiento (Persistencia en BD) --}}
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                <x-input-label for="latitude" value="Latitud" />
                <x-text-input id="latitude" name="latitude" type="text" class="mt-1 block w-full bg-gray-100" :value="old('latitude', $user->latitude)" readonly />
            </div>
            <div>
                <x-input-label for="longitude" value="Longitud" />
                <x-text-input id="longitude" name="longitude" type="text" class="mt-1 block w-full bg-gray-100" :value="old('longitude', $user->longitude)" readonly />
            </div>
        </div>
        ...
        <div>
            ...
            <x-text-input id="address" name="address" type="text" class="mt-1 block w-full bg-gray-50" :value="old('address', $user->address)" readonly />
        ...
        <div class="flex items-center gap-4">
            ...
            @if (session('status') === 'profile-updated')
                ...
                >{{ __('Ubicación guardada correctamente.') }}</p>
            @endif
        </div>
    </form>
